...
def CATD(Dataset,iti,rate):
    ...
    while(t<=iti):
        ...
        for i in range(nos):
            if ((num[i] - 1) == 0):
                a = S_value_mean[i] - (st.t.ppf(1-rate/2, num[i] ) * np.sqrt(S_value_error[i])) / np.sqrt(num[i]);
                b = S_value_mean[i] + (st.t.ppf(1-rate/2, num[i]) * np.sqrt(S_value_error[i])) / np.sqrt(num[i]);
                a1=S_value_mean1[i] - (st.t.ppf(1-rate/2, num[i] ) * np.sqrt(S_value_error1[i])) / np.sqrt(num[i]);
                b1 = S_value_mean1[i] + (st.t.ppf(1-rate/2, num[i]) * np.sqrt(S_value_error1[i])) / np.sqrt(num[i]);
            else:
                a=S_value_mean[i]-(st.t.ppf(1-rate/2,num[i]-1)*np.sqrt(S_value_error[i]))/np.sqrt(num[i]);
                a1 = S_value_mean1[i] - (st.t.ppf(1-rate/2, num[i] - 1) * np.sqrt(S_value_error1[i])) / np.sqrt(num[i]);
                b = S_value_mean[i] + (st.t.ppf(1-rate/2, num[i] - 1) * np.sqrt(S_value_error[i])) / np.sqrt(num[i]);
                b1 = S_value_mean1[i] + (st.t.ppf(1-rate/2, num[i] - 1) * np.sqrt(S_value_error1[i])) / np.sqrt(num[i]);
            interval[i][0]=a;
            interval1[i][0] = a1;
            interval[i][1]=b;
            interval1[i][1] = b1;
        ...
        for i in range(nos):
            ...
            interval_mean[i] = 1 / np.abs(interval[i][0]);
            interval_mean1[i] = 1 / np.abs(interval1[i][0]);
        ...
        for i in range(noe):
            ...
            Truth[i][1] = round(truth[i][0]);
            Truth[i][2] = round(truth1[i][0]);
