#near算法是将附近30米半径做圆范围内的点求均值做真值发现
import math
...
df1 = pd.DataFrame(pd.read_excel('translation_data/Bus_data_06/117.xlsx'));
Uni_Bus_ID = list(df1['BUS_ID'].unique());
Uni_Bus_ID.sort();
v1=df1.values;
df2=pd.DataFrame(pd.read_excel('translation_data/truth_line/109_truth.xlsx'));
v2=df2.values;
Trans=np.array([np.array([0,'0',0.0,0.0]) for i in range(v2.__len__()*12)]);#保存数据
#Trans=[0,'0',0.0,0.0];
m=0;
...
with open('bus_station_with_subway_data/109_Trans.csv', 'w+') as f:
...
truth_and_weight_EXP=cbs.CBS(Data, 50);
...
#提取相关站点
bus_station_with_subway=['桐梓坡','六沟垅','四水厂','望月湖小区','太平街口','南阳街口'];

#bus_station_with_subway_data=np.array([np.array(['0',0.0,0.0]) for i in range(len(bus_station_with_subway))]);
with open('bus_station_with_subway_data/109_CRH.csv', 'w') as f:
...
with open('bus_station_with_subway_data/109_CATD.csv', 'w') as f:
    for i in range(bus_station_with_subway.__len__()):
...
with open('bus_station_with_subway_data/109_TMVD.csv', 'w') as f:
    for i in range(bus_station_with_subway.__len__()):
...
with open('bus_station_with_subway_data/109_GTM.csv', 'w') as f:
    for i in range(bus_station_with_subway.__len__()):
...
with open('bus_station_with_subway_data/109_KDEm.csv', 'w') as f:
    for i in range(bus_station_with_subway.__len__()):
...
with open('bus_station_with_subway_data/109_Mean.csv', 'w') as f:
    for i in range(bus_station_with_subway.__len__()):
...
with open('bus_station_with_subway_data/109_Median.csv', 'w') as f:
    for i in range(bus_station_with_subway.__len__()):
