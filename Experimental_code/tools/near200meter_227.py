...
df1 = pd.DataFrame(pd.read_excel('../translation_data/Bus_data_06/227.xlsx'));
Uni_Bus_ID = list(df1['BUS_ID'].unique());
Uni_Bus_ID.sort();
v1=df1.values;
df2=pd.DataFrame(pd.read_excel('../translation_data/truth_line/142_truth.xlsx'));
v2=df2.values;
Trans=np.array([np.array([0,'0',0.0,0.0]) for i in range(v2.__len__()*37)]);#保存数据
#Trans=[0,'0',0.0,0.0];
m=0;

for i in range(len(Uni_Bus_ID)):
    if(Uni_Bus_ID[i]==241049):
        continue;
...
# with open('F:\PythonWorkplace/bus_station_with_subway_data/142_Trans.csv','w+') as f:
...
#
with open('test/bus_station_with_subway_data/all_bus_stop/142_CRH.csv', 'w') as f:
    for j in range(len(truth_and_weight_CRH)):
            f.write(truth_and_weight_CRH[j][0]+','+truth_and_weight_CRH[j,1]+','
                    +truth_and_weight_CRH[j,2]+'\n');

with open('test/bus_station_with_subway_data/all_bus_stop/142_CATD.csv', 'w') as f:
...
with open('test/bus_station_with_subway_data/all_bus_stop/142_Mean.csv', 'w') as f:
...
with open('test/bus_station_with_subway_data/all_bus_stop/142_MEDIAN.csv', 'w') as f:
...
#提取相关站点
bus_station_with_subway=['华夏首末站','蓉园路口','洪西村','芙蓉区卫计局','省血液中心','朝阳村','马栏山','月湖公园',
                         '特立西路口','机电职院西','荣盛花语城体育公园'];

#bus_station_with_subway_data=np.array([np.array(['0',0.0,0.0]) for i in range(len(bus_station_with_subway))]);
with open('test/bus_station_with_subway_data/142_CRH.csv', 'w+') as f:
...
with open('test/bus_station_with_subway_data/142_CATD.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_CATD)):
...
with open('test/bus_station_with_subway_data/142_TMVD.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_EXP)):
...
with open('test/bus_station_with_subway_data/142_GTM.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_GTM)):
...
with open('test/bus_station_with_subway_data/142_KDEm.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_KDEm)):
...
with open('test/bus_station_with_subway_data/142_Mean.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_MEAN)):
...
with open('test/bus_station_with_subway_data/142_Median.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_MEDIAN)):
