...
df1 = pd.DataFrame(pd.read_excel('../translation_data/Bus_data_06/239.xlsx'));
Uni_Bus_ID = list(df1['BUS_ID'].unique());
Uni_Bus_ID.sort();
v1=df1.values;
df2=pd.DataFrame(pd.read_excel('../translation_data/truth_line/802_truth.xlsx'));
v2=df2.values;
Trans=np.array([np.array([0,'0',0.0,0.0]) for i in range(v2.__len__()*44)]);#保存数据
#Trans=[0,'0',0.0,0.0];
m=0;

for i in range(len(Uni_Bus_ID)):
    if(Uni_Bus_ID[i]==240068 or Uni_Bus_ID[i]==240082 or Uni_Bus_ID[i]==240083 or Uni_Bus_ID[i]==240088):
        continue;
    ...
    for j in range(v2.__len__()):
        ...
        for k in range(v3.__len__()):
            lon2=v3[k][5]*pow(10,-6);
            lat2=v3[k][6]*pow(10,-6);
        ...
        if (len(tmep_dis_letter_200) != 0):
            ...
            Trans[m][1] = v2[j][0];
            #a = np.mean(tmep_dis_letter_200[:, 5].astype(float));
            Trans[m][2] = round(np.mean(tmep_dis_letter_200[:, 5].astype(float)));
            Trans[m][3] = round(np.mean(tmep_dis_letter_200[:, 6].astype(float)));
            m = m + 1;
        else:
            Trans[m][0] = Uni_Bus_ID[i];
            Trans[m][1] = v2[j][0];
...
# with open('F:\PythonWorkplace/bus_station_with_subway_data/802_Trans.csv','w+') as f:
...
#
with open('test/bus_station_with_subway_data/all_bus_stop/802_CRH.csv', 'w') as f:
    for j in range(len(truth_and_weight_CRH)):
            f.write(truth_and_weight_CRH[j][0]+','+truth_and_weight_CRH[j,1]+','
                    +truth_and_weight_CRH[j,2]+'\n');

with open('test/bus_station_with_subway_data/all_bus_stop/802_CATD.csv', 'w') as f:
...
with open('test/bus_station_with_subway_data/all_bus_stop/802_Mean.csv', 'w') as f:
...
with open('test/bus_station_with_subway_data/all_bus_stop/802_MEDIAN.csv', 'w') as f:
...
#提取相关站点
bus_station_with_subway=['远大路口','万家丽广场','芙蓉区政府','石坝路口','中城丽景香山',
                         '新屋里','湘府中学','大托铺','陶家山','市交通运输局',
                         '高桥大市场东','长沙大道口','曲塘路口','豆花塘','湖南联通',
                         '师家老屋'];

with open('test/bus_station_with_subway_data/802_CRH.csv', 'w+') as f:
...
with open('test/bus_station_with_subway_data/802_CATD.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_CATD)):
...
with open('test/bus_station_with_subway_data/802_TMVD.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_EXP)):
...
with open('test/bus_station_with_subway_data/802_GTM.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_GTM)):
...
with open('test/bus_station_with_subway_data/802_KDEm.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_KDEm)):
...
with open('test/bus_station_with_subway_data/802_Mean.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_MEAN)):
...
with open('test/bus_station_with_subway_data/802_Median.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_MEDIAN)):
