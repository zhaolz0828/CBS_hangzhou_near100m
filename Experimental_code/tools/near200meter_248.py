...
df1 = pd.DataFrame(pd.read_excel('../translation_data/Bus_data_06/248.xlsx'));
Uni_Bus_ID = list(df1['BUS_ID'].unique());
Uni_Bus_ID.sort();
v1=df1.values;
df2=pd.DataFrame(pd.read_excel('../translation_data/truth_line/810_truth.xlsx'));
v2=df2.values;
Trans=np.array([np.array([0,'0',0.0,0.0]) for i in range(v2.__len__()*23)]);#保存数据
#Trans=[0,'0',0.0,0.0];
m=0;

for i in range(len(Uni_Bus_ID)):
    if(Uni_Bus_ID[i]==240305 or Uni_Bus_ID[i]==240326):
        continue;
...
# with open('F:\PythonWorkplace/bus_station_with_subway_data/810_Trans.csv','w+') as f:
...
#
with open('test/bus_station_with_subway_data/all_bus_stop/810_CRH.csv', 'w') as f:
    for j in range(len(truth_and_weight_CRH)):
            f.write(truth_and_weight_CRH[j][0]+','+truth_and_weight_CRH[j,1]+','
                    +truth_and_weight_CRH[j,2]+'\n');

with open('test/bus_station_with_subway_data/all_bus_stop/810_CATD.csv', 'w') as f:
...
with open('test/bus_station_with_subway_data/all_bus_stop/810_Mean.csv', 'w') as f:
...
with open('test/bus_station_with_subway_data/all_bus_stop/810_MEDIAN.csv', 'w') as f:
...
#提取相关站点
bus_station_with_subway=['甘家老屋','马栏山','朝阳村','省血液中心','火炬村','省老干所',
                         '马王堆医院','马王堆汽配大市场','远大路口','万家丽广场','芙蓉区政府',
                         '市交通运输局','高桥大市场东','长沙大道口','石坝路口','劳动东路口',
                         '中城丽景香山','曲塘路口','豆花塘','湖南联通','煤田地质局','省环保厅',
                         '唐湘电器市场'];

with open('test/bus_station_with_subway_data/810_CRH.csv', 'w+') as f:
...
with open('test/bus_station_with_subway_data/810_CATD.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_CATD)):
...
with open('test/bus_station_with_subway_data/810_TMVD.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_EXP)):
...
with open('test/bus_station_with_subway_data/810_GTM.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_GTM)):
...
with open('test/bus_station_with_subway_data/810_KDEm.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_KDEm)):
...
with open('test/bus_station_with_subway_data/810_Mean.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_MEAN)):
...
with open('test/bus_station_with_subway_data/810_Median.csv', 'w+') as f:
    for i in range(bus_station_with_subway.__len__()):
        for j in range(len(truth_and_weight_MEDIAN)):
